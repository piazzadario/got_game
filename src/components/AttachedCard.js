    ...
    return (
        <div onMouseOver={() => { setAttachmentVisible(true) }} onMouseLeave={() => setAttachmentVisible(false)} onClick={() => props.onKnee()}
            ...
            <img key={idx} style={{ height: '100%', width: '100%' }}
                className={card.isKneed ? 'kneed' : ''}
                src={`https://lcg-cdn.fantasyflightgames.com/got2nd/GT${card.id}.jpg`}
                alt='teste'></img>
            {attachmentVisible  && <div className='card-options'>
                ...
                <Button className='py-0' variant='success' onClick={(ev)=>{props.onShowCardInfo(card.id); ev.stopPropagation()}}>INFO</Button>
            </div>}
        </div>
    );
}
