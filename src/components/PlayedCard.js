...
const PlayedCard = (props) => {
    ...
    return (
        ...
        <Col sm={2} style={{ position: 'relative', display: 'table', width:'12.5%', maxWidth:'12.5%' }} className='mb-1 mx-2 px-0'>
            <div onMouseOver={() => setVisible(true)} onMouseLeave={() => setVisible(false)} onClick={() => {if(props.owner) props.onKnee(card.id,from)}} style={{ position: 'relative', zIndex: '20', width:'100%'}}>
